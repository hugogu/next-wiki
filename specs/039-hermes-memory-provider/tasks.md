...
forget records through immutable Raw entries and the shared Wiki
page/revision/content/index lifecycle.
...
- [X] T090 [US3] Make conversation evidence appendable across lifecycle events by deriving one Raw conversation page from destination, agent identity, and session digest; persist each capture's exact Raw revision and preserve retry idempotency in `apps/web/src/server/services/agent-memory.ts`, `apps/web/src/server/db/schema/agent-memory.ts`, and `apps/web/src/server/db/migrations/0022_slow_red_skull.sql`.
