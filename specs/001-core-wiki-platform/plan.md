...
in-PostgreSQL); unified/remark/rehype (Markdown pipeline); CodeMirror 6
(client Markdown editor); Tailwind CSS + CSS custom properties + in-house
`src/components/ui/` primitives (unified design system, no third-party component
library); TanStack Query (client server state); Zustand (UI state);
React Hook Form (forms); Zod (schemas); REST route handlers + OpenAPI (HTTP API). **i18n**: custom
...
Markdown to HTML; client editor serializes to raw Markdown only (CodeMirror 6).
...
| P5 Style System & UI Consistency | PASS | All UI built on the unified design system in `src/components/ui/` (in-house primitives + Tailwind + CSS-variable tokens; no third-party component library). No per-page bespoke styling. |
...
| Mandate: Editor Extensibility | PASS | CodeMirror 6 write-side; serialize to raw Markdown only; client editor state never leaves browser; server parses raw Markdown via remark into an independent AST. Markdown is the default editor. |
...
└── src/
    ...
    ├── components/
    │   ├── ui/                         # Unified design system (in-house primitives + tokens)
    │   ├── editor/                     # CodeMirror 6 Markdown editor (client)
...
|---|---|---|
| Role-based permissions only (no per-page permission entries yet) | Spec A7 scopes this slice to 3 roles + auth