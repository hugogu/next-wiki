...
- **FR-043**: A Wiki Linker proposal MUST show each proposed link's keyword, its location in the page, and its target page, and MUST be acceptable or rejectable as one reviewable change.
...
- **Review policy**: This feature introduces no new review model. It reuses the draft/diff path for page-content changes and the change-proposal path for tag, metadata, and link changes established in earlier features.
