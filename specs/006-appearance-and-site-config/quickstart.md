...
```bash
pnpm install
pnpm db:migrate            # applies 0017_appearance_site_config + built-in theme seed
...
**Code audit** (SC-001):
```bash
# No raw hex/rgb or px/rem font-size literals in feature components
rg -n "#[0-9a-fA-F]{3,8}\b|rgb\(|font-size:\s*\d" apps/web/src/components apps/web/app \
  --glob '!**/ui/**' --glob '!**/globals.css'
# Expect: no color/font-size literals in feature (non-ui) components
```
...
1. **Admin/Profile → Appearance → Markdown Themes** (per-user surface).
...
5. Activate the personal theme → an article re-renders with new typography within
   3s; colors stay consistent with the system theme (FR-011a, SC-004).
