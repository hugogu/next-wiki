...
- [ ] T006 [P] Add failing schema/index coverage for binding purposes and the `agent_memory_wiki_documents` projection in `apps/web/src/server/db/agent-memory-schema.test.ts`.
...
- [ ] T010 Define the `memory_provider`, `mirror`, and `knowledge_search` binding-purpose enum; add the source-path/digest/revision locator table and indexes; and register the schema in `apps/web/src/server/db/schema/enums.ts`, `apps/web/src/server/db/schema/agent-memory.ts`, and `apps/web/src/server/db/schema/index.ts`.
...
- [ ] T014 Implement an internal trusted complete-Raw-snapshot revision helper that retains verbatim Markdown, standard rendering/index/asset/replication behavior, and immutable prior revisions in `apps/web/src/server/services/raw-entries.ts`.
...
- [ ] T016 [P] [US1] Add service tests for source-path validation, reversible server path projection, case-fold collisions, exact frontmatter/body/link retention, digest/idempotency conflicts, concurrent upserts, unchanged results, changed full snapshots, and source-removal retention in `apps/web/src/server/services/agent-memory-wiki.test.ts`.
...
- [ ] T021 [US1] Implement source-path normalization/collision detection, server-derived remote-path projection, locator locking, provenance shaping, and idempotent Raw-snapshot persistence in `apps/web/src/server/services/agent-memory-wiki.ts`.
...
- [ ] T023 [US1] Implement bounded `PUT /api/v1/memory/wiki/documents` validation and created/updated/unchanged snapshot responses in `apps/web/app/api/v1/memory/wiki/documents/route.ts` using `apps/web/src/server/services/agent-memory-wiki.ts`.
...
- [ ] T031 [P] [US2] Add knowledge-facade service tests for readable-space fan-out, safe coverage, ranking/citation shaping, empty results, Raw/Generated omission, and page-read reauthorization in `apps/web/src/server/services/agent-memory-wiki-search.test.ts`.
...
- [ ] T035 [US2] Implement the knowledge-search and single-page-read facade that delegates to existing public-content visibility/ranking services, recomputes coverage per request, exposes source provenance only when readable, and bounds returned Markdown in `apps/web/src/server/services/agent-memory-wiki.ts`.
...
- Keep markdown canonical only in Raw revisions; locator/journal/audit state
  must not store bodies, queries, excerpts, or secrets.
...
- After Foundation, US1 mirror implementation and US2 read facade/tests can
  be assigned separately if changes to `apps/web/src/server/services/agent-memory-wiki.ts`
  are coordinated.
...
```text
Task: "T016 service snapshot/provenance tests in apps/web/src/server/services/agent-memory-wiki.test.ts"
