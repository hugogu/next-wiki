...
The existing `content_type` column used by `page_revisions.content_type` is **broadened from a closed `text/markdown` enum to an open content-type string** (no longer a closed enum). Wiki and generated pages keep defaulting to `text/markdown`; raw entries use the original source format (`text/plain`, `text/html`, `application/json`, `application/pdf`, `text/x-log`, `image/*`, etc.). The column type changes from enum to `text` with `CHECK (content_type IS NOT NULL)` plus a structural `CHECK` enforcing RFC 2046 grammar (`type "/" subtype ["+" tree] [";" params]` — params stripped before storage), so the DB rejects malformed MIME types without hardcoding an allowlist. The service layer parses values with a standard MIME-type library (chosen at implementation time — e.g. `content-type` or `mime-types`) and MAY apply a curated allowlist for raw creates while leaving the column open for future formats. Previously stored `text/markdown` rows remain valid.
...
| `raw_category_id` | uuid NULL FK → `raw_categories.id` ON DELETE RESTRICT | — | Raw entries only; immutable after creation. NULL for non-raw pages. Constrained NOT NULL at the service layer on raw create (no DB-level NOT NULL — see D15 for the rationale: backfill is admin-driven, and the column is NULL for non-raw pages by design). Since the deployment has no historical raw data on first run of this feature, no backfill migration is needed; admin must create at least one category before raw writes succeed (or set `is_default=true` to have it applied silently) |
...
| Raw entry `raw_category_id` MUST be assigned at create (or via admin-configured default); immutable thereafter | `services/raw-entries.ts` create path + `pages.updateProperties` reject for raw. DB column is nullable so non-raw pages can legitimately be NULL; the raw NOT NULL rule is service-layer only. Since this feature ships with no historical raw data, no backfill is needed. |
...
| Original-byte assets referenced by `page_revisions.original_asset_id` MUST be protected from accidental cleanup — the existing orphan-cleanup job (`apps/web/src/server/jobs/orphan-cleanup.ts`) soft-deletes abandoned uploads; its reference graph MUST be extended to include `original_asset_id` so raw original bytes are never reclaimed while still referenced | `services/raw-entries.ts` write path creates the asset before the revision; `jobs/orphan-cleanup.ts` extended to skip any asset id appearing in `page_revisions.original_asset_id` |
| `content_type` MUST match RFC 2046 grammar; service-layer parsing with a standard MIME library | `CHECK` constraint on `page_revisions.content_type` + Zod schema in `packages/shared` |
