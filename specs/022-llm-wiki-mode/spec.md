...
- **FR-007b**: Raw entries MUST be stored in a dual-track form: an **extracted-text representation** suitable for retrieval, rendering, and AI consumption (stored as `page_revisions.content_source`, format indicated by `content_type`), and — when an original byte payload exists (PDF, HTML, JSON export, image, raw log) — the **original bytes** stored through the existing `content_assets` abstraction with an immutable reference from the revision. The extracted text is the default surface for search (lexical and semantic) and for AI pipelines; the original bytes are the default surface for human verbatim viewing and download. Both layers MUST reuse the existing 003 content-store architecture (Database/Local/S3 backends) without introducing a new storage subsystem.
