...
- [ ] T001 Create detector fixture directory with OpenRouter and Cloudflare sample payload placeholders in `apps/web/src/server/ai/model-detectors/__fixtures__/README.md`
- [ ] T002 [P] Add detector-focused Vitest fixture helpers scaffold in `apps/web/src/server/ai/model-detectors/test-helpers.ts`
...
- [ ] T004 Add shared detector source, detector config, and sync action result schemas in `packages/shared/src/ai.ts`
- [ ] T005 Update AI provider vendor/protocol definitions for detector source selection in `packages/shared/src/ai.ts`
- [ ] T006 [P] Add detector contract types for runtime config, detected models, warnings, and list results in `apps/web/src/server/ai/model-detectors/types.ts`
- [ ] T007 [P] Add detector registry with explicit `openrouter` and `cloudflare` registration points in `apps/web/src/server/ai/model-detectors/registry.ts`
- [ ] T008 Move current OpenRouter detector behavior behind the new detector contract in `apps/web/src/server/ai/model-detectors/openrouter.ts`
- [ ] T009 Replace legacy exports with compatibility wrappers or re-exports in `apps/web/src/server/ai/model-detector.ts`
- [ ] T010 Add detector config parsing and credential resolution helpers in `apps/web/src/server/services/ai-admin.ts`
- [ ] T011 Add action-backed model-sync enqueue/resume service functions in `apps/web/src/server/services/ai-admin.ts`
- [ ] T012 Add or update model-sync worker handling for detector-backed runs in `apps/web/src/server/services/ai-actions.ts`
- [ ] T013 Update provider create/update validation for detector config and write-only credentials in `apps/web/src/server/services/ai-admin.ts`
- [ ] T014 [P] Add shared i18n keys for detector source, status, partial enrichment, and provenance labels in `apps/web/src/i18n/keys.ts`
- [ ] T015 [P] Add English detector labels and messages in `apps/web/messages/en.json`
- [ ] T016 [P] Add Chinese detector labels and messages in `apps/web/messages/zh.json`
...
- [ ] T017 [P] [US1] Add Cloudflare detector mapping tests for model search, schema enrichment, modalities, and deprecation in `apps/web/src/server/ai/model-detectors/cloudflare.test.ts`
- [ ] T018 [P] [US1] Add Cloudflare partial schema failure tests in `apps/web/src/server/ai/model-detectors/cloudflare.test.ts`
- [ ] T019 [P] [US1] Add Cloudflare provider config validation and secret-redaction route tests in `apps/web/app/api/ai/ai-admin-routes.test.ts`
- [ ] T020 [P] [US1] Add Cloudflare model sync service tests for added, updated, unavailable, skipped, and partial counts in `apps/web/src/server/services/ai-admin-detectors.test.ts`

### Implementation for User Story 1

- [ ] T021 [US1] Implement Cloudflare detector list and schema fetch client with bounded timeout and sanitized errors in `apps/web/src/server/ai/model-detectors/cloudflare.ts`
- [ ] T022 [US1] Implement Cloudflare catalog-to-model normalization in `apps/web/src/server/ai/model-detectors/cloudflare.ts`
- [ ] T023 [US1] Implement Cloudflare schema-to-capability mapping without model-name inference in `apps/web/src/server/ai/model-detectors/cloudflare.ts`
- [ ] T024 [US1] Implement per-model partial warning collection and non-blocking schema enrichment failure handling in `apps/web/src/server/ai/model-detectors/cloudflare.ts`
- [ ] T025 [US1] Wire Cloudflare detector source selection into provider runtime config resolution in `apps/web/src/server/services/ai-admin.ts`
- [ ] T026 [US1] Merge Cloudflare detected models into `ai_models` and `ai_model_capabilities` while preserving unseen model availability semantics in `apps/web/src/server/services/ai-admin.ts`
- [ ] T027 [US1] Update model sync route to return queued/resumed `model_sync` action for detector-backed providers in `apps/web/app/api/ai/providers/[id]/model-syncs/route.ts`
- [ ] T028 [US1] Add Cloudflare detector fields to provider form validation and submission payload in `apps/web/src/components/admin/ai/ProviderForm.tsx`
- [ ] T029 [US1] Add Cloudflare detector source labels and validation messages to provider form UI in `apps/web/src/components/admin/ai/ProviderForm.tsx`
...
- [ ] T030 [P] [US2] Add detector registry tests for explicit source lookup and unknown source rejection in `apps/web/src/server/ai/model-detectors/registry.test.ts`
- [ ] T031 [P] [US2] Add OpenRouter detector contract regression tests for namespace filtering, chat capabilities, embedding models, and output modality filtering in `apps/web/src/server/ai/model-detectors/openrouter.test.ts`
- [ ] T032 [P] [US2] Add sync orchestration tests proving OpenRouter and Cloudflare both flow through normalized `DetectedModel` merge in `apps/web/src/server/services/ai-admin-detectors.test.ts`
...
- [ ] T038 [US2] Update shared model sync result schema to include detector counts and action identifiers in `packages/shared/src/ai.ts`
...
- [ ] T039 [P] [US3] Add manual override precedence tests across OpenRouter and Cloudflare syncs in `apps/web/src/server/services/ai-admin-detectors.test.ts`
...
- [ ] T042 [US3] Ensure detector merge skips overwriting `manually_added` model rows in `apps/web/src/server/services/ai-admin.ts`
- [ ] T043 [US3] Ensure detector merge upserts only detector-owned capability rows and never deletes `source=manual` rows in `apps/web/src/server/services/ai-admin.ts`
...
- [ ] T047 [P] [US4] Add normalized detector error and secret-redaction tests in `apps/web/src/server/ai/model-detectors/errors.test.ts`
- [ ] T048 [P] [US4] Add action metadata tests for detector source, freshness, counts, warnings, and sanitized failures in `apps/web/src/server/services/ai-admin-detectors.test.ts`
...
- [ ] T051 [US4] Implement detector error normalization and safe detail shaping in `apps/web/src/server/ai/model-detectors/types.ts`
- [ ] T052 [US4] Persist detector source, freshness, counts, warnings, and sanitized failure metadata on `model_sync` actions in `apps/web/src/server/services/ai-admin.ts`
...
- [ ] T056 [US4] Ensure all detector status and model sync admin routes enforce `manage_ai` and return safe errors in `apps/web/app/api/ai/providers/[id]/model-syncs/route.ts`
...
- [ ] T057 [P] Update OpenAPI schema tests for changed model sync response shape in `apps/web/src/server/api/openapi-schemas.test.ts`
- [ ] T058 [P] Regenerate or update generated OpenAPI output for AI admin routes in `apps/web/public/openapi.json`
- [ ] T059 [P] Run i18n validation and fix detector translation key coverage in `apps/web/src/i18n/keys.ts`
...
- [ ] T063 Run lint and typecheck, fixing final issues in `packages/shared/src/ai.ts`
