...
- [ ] T003 Record the feature-specific Docker, migration, focused-test, OpenAPI, and i18n validation commands in `specs/027-request-logging/quickstart.md` and keep them aligned with the implementation paths.
...
- [ ] T004 [P] Define shared request-log enums, source descriptors, settings schemas, list filters, list summaries, detail envelopes, and API error shapes in `packages/shared/src/request-log.ts`.
- [ ] T005 [P] Export the request-log shared types and schemas from `packages/shared/src/index.ts`.
- [ ] T006 [P] Add `request_log_level` and request outcome enum definitions in `apps/web/src/server/db/schema/enums.ts`, following the existing Drizzle enum conventions.
- [ ] T007 Define the singleton `request_log_settings` and append-only `outbound_request_logs` tables, encrypted columns, nullable-response semantics, retention columns, foreign-key attribution, and all list/cleanup indexes in `apps/web/src/server/db/schema/request-logs.ts`.
- [ ] T008 Re-export the request-log schema from the main Drizzle schema entry point in `apps/web/src/server/db/schema/index.ts`.
- [ ] T009 Generate the Drizzle migration and matching snapshot/journal entries from the edited schema using `pnpm db:generate`, writing generated artifacts only under `apps/web/src/server/db/migrations/`; do not hand-author SQL or metadata.
- [ ] T010 Re-run `pnpm db:generate` and verify the schema is clean with no changes reported, using `apps/web/src/server/db/schema/request-logs.ts` as the source of truth.
- [ ] T011 Add the dedicated `manage_request_logs` action and `request_logs` resource, including Admin-only and API-key-denied behavior, in `apps/web/src/server/permissions/index.ts`.
...
- [ ] T013 Implement the generic best-effort capture service in `apps/web/src/server/services/request-log.ts`, including the explicit source descriptor, one capture decision per attempt, Status/Header/All field boundaries, ordered duplicate header values, absent-versus-empty body markers, safe target metadata, normalized outcome/error summary, correlation and attempt fields, encryption/decryption through the existing AES-256-GCM helpers, and non-recursive failure isolation.
- [ ] T014 Add unit coverage for disabled capture, level conformance, encryption, body/header preservation, correlation, retries, pre-response failures, logging-write failure isolation, and expiry calculation in `apps/web/src/server/services/request-log.test.ts`.
- [ ] T015 Add shared settings/list/detail validation schemas and scanner-compatible response definitions in `apps/web/src/server/api/openapi-schemas.ts`, keeping raw encrypted fields out of list schemas.
...
- [ ] T016 [P] [US1] Add settings transition tests for default values, retention bounds, All confirmation, level snapshots, audit attribution input, and turning capture off without deleting existing rows in `apps/web/src/server/services/request-log-settings.test.ts`.
...
- [ ] T019 [US1] Implement settings read/update, singleton initialization defaults, retention validation, sensitive-capture confirmation, immutable per-attempt level selection, and settings-change audit context in `apps/web/src/server/services/request-log.ts`.
- [ ] T020 [US1] Implement the Admin-only dynamic settings route with `createApiContext`, permission checks, shared schemas, `withApiAudit`, `mapDomainError`, no-cache behavior, and OpenAPI annotations in `apps/web/app/api/request-log/settings/route.ts`.
- [ ] T021 [US1] Refactor the complete existing AI provider operation boundary so capture wraps fetch, response parsing, and stream consumption while preserving the original consumer stream, timeout, cancellation, retry, and error behavior in `apps/web/src/server/ai/providers/http-client.ts`.
...
- [ ] T023 [US1] Update the existing AI provider call sites to pass the explicit request-log descriptor and complete operation callback without serializing request bodies or consulting capture storage when capture is disabled in `apps/web/src/server/ai/providers/http-client.ts` and `apps/web/src/server/ai/providers/*.ts`.
...
- [ ] T024 [P] [US2] Add list/detail service tests for bounded pagination, stable newest-first ordering, every documented filter, expiry exclusion, safe list projections, Admin-only decryption, and not-found behavior for expired/unauthorized records in `apps/web/src/server/services/request-log-routes.test.ts`.
- [ ] T025 [P] [US2] Add API route contract tests for list and detail responses, including all query filters, maximum page size, no raw payloads in list results, no-cache headers, and unified unauthorized/not-found behavior in `apps/web/src/server/services/request-log-routes.test.ts`.
...
- [ ] T027 [US2] Implement request-log list and detail query services with metadata-only list selection, bounded filters/page sizes, deterministic `(created_at, id)` ordering, expiry predicates, Admin permission checks, and detail decryption in `apps/web/src/server/services/request-log.ts`.
- [ ] T028 [US2] Implement the dynamic Admin-only paginated list route with all documented filters, shared response schemas, audit wrapper, no-cache behavior, and OpenAPI annotations in `apps/web/app/api/request-log/route.ts`.
- [ ] T029 [US2] Implement the dynamic Admin-only detail route that returns complete level-allowed values only after authorization, preserves unavailable-versus-empty semantics, and maps unknown/expired/unauthorized records without existence disclosure in `apps/web/app/api/request-log/[id]/route.ts`.
...
- [ ] T032 [US2] Add the canonical Admin list and detail pages with dynamic rendering, current-list back navigation, and existing Admin layout primitives in `apps/web/app/(admin)/admin/request-log/page.tsx` and `apps/web/app/(admin)/admin/request-log/[id]/page.tsx`.
- [ ] T033 [US2] Add exactly one `REQUEST LOG` item to the existing `DATA & OPERATIONS` navigation group and wire the page labels, settings, filters, outcomes, columns, warnings, sensitive sections, empty states, accessibility labels, and permission messages in `apps/web/src/components/layout/Navigator.tsx`, `apps/web/src/i18n/keys.ts`, `apps/web/messages/en.json`, and `apps/web/messages/zh.json`.
...
- [ ] T037 [US3] Make the public server-side capture contract explicit and bounded for future HTTP or SDK integrations, including source registration, operation callback ownership, correlation, attempts, and recursion guard in `apps/web/src/server/services/request-log.ts`.
- [ ] T038 [US3] Ensure every existing AI provider adapter uses the same source descriptor and request wrapper rather than provider-specific persistence or viewer logic in `apps/web/src/server/ai/providers/openai-compatible.ts`, `apps/web/src/server/ai/providers/openrouter.ts`, `apps/web/src/server/ai/providers/anthropic.ts`, `apps/web/src/server/ai/providers/voyage.ts`, and `apps/web/src/server/ai/providers/minimax.ts`.
- [ ] T039 [US3] Add a test-only fixture integration that adopts the common wrapper and verify its records share the AI list/detail schemas without adding a new table, route, filter model, or UI surface in `apps/web/src/server/services/request-log.test.ts` and `apps/web/e2e/request-log.spec.ts`.
...
- [ ] T044 [US4] Reuse the existing API audit wrapper for settings/list/detail access and record previous/new capture policy values with the acting Admin in `apps/web/src/server/api/audit-wrapper.ts`, `apps/web/src/server/services/audit.ts`, and `apps/web/app/api/request-log/settings/route.ts`.
- [ ] T045 [US4] Extend the existing scheduled cleanup path to delete expired request-log rows in bounded batches, without touching pages, revisions, AI actions, or audit entries, in `apps/web/src/server/jobs/ai-cleanup.ts` and `apps/web/src/server/jobs/register.ts`.
...
- [ ] T048 Add the three request-log route groups and shared schemas to the scanner-compatible OpenAPI definitions, then regenerate the committed document with `pnpm --filter @next-wiki/web openapi:generate` in `apps/web/src/server/api/openapi-schemas.ts` and `apps/web/public/openapi.json`.
- [ ] T049 Validate English and Simplified Chinese completeness, accessible labels/keyboard behavior, and absence of browser alert popups for the request-log surface in `apps/web/src/i18n/keys.ts`, `apps/web/messages/en.json`, `apps/web/messages/zh.json`, and `apps/web/src/components/admin/request-log/RequestLogPanel.tsx`.
...
- [ ] T051 Run `pnpm --filter @next-wiki/web lint`, `pnpm --filter @next-wiki/web typecheck`, `pnpm --filter @next-wiki/web i18n:validate`, and the generated-migration no-change check documented in `apps/web/package.json` and `apps/web/src/server/db/migrations/`.
...
```text
T004-T005 shared schemas and exports
...
- No new service, queue, cache, or vendor SDK is introduced.
