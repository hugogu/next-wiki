...
**Decision**: Add a new API-key scope `ai.read` that maps to the existing `use_ai_search` permission action (and, when Q&A ships, to `use_ai_qa`). The new scope is added in six places: the shared Zod schema (`packages/shared/src/api-keys.ts:3-13`), the DB pgEnum (`apps/web/src/server/db/schema/enums.ts:7-17`), the `scopeToActions` table (`apps/web/src/server/permissions/index.ts:58-68`), the api-key hard-deny list at `apps/web/src/server/permissions/index.ts:132-144`, the API-key creation dialog's `SCOPE_ORDER` (`apps/web/src/components/user-center/ApiKeyCreateDialog.tsx:11`), and the locale files for display text. The api-key hard-deny list is updated to remove `use_ai_search` and `use_ai_qa` (with a comment explaining the carve-out is gated by the new scope).

**Rationale**: The existing `use_ai_search` and `use_ai_qa` permission actions already exist in the codebase (`apps/web/src/server/permissions/index.ts:39-40`); no new action constant is required. The `view` scope is necessary for result-level page reads, but it is too coarse as the only semantic-search gate because it would conflate ordinary content reads with AI retrieval. A dedicated `ai.read` scope is the only way to satisfy Constitution P5 ("every API route MUST check permissions") while keeping the new endpoints behind a per-capability key grant. Public semantic search therefore requires `view + ai.read`: `view` for page visibility and `ai.read` for the AI retrieval action. The migration is a single `ALTER TYPE` to add a value to the `apiKeyScopeEnum` (Drizzle generates it from `pnpm db:generate` per `AGENTS.md`).

**Alternatives considered**:

- **Reuse `view` as the only gate**: rejected — it would let any content-read key invoke AI retrieval. The granularity of the API-key scope model is meant to be a feature, not a workaround.
