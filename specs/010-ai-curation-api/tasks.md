...
> **Path convention**: this is a Next.js monorepo. "Backend" = `apps/web/src/server/` and `apps/web/app/api/v1/`. "Shared" = `packages/shared/src/`. "MCP" = `packages/mcp-server/src/`. Database = Drizzle ORM, `apps/web/src/server/db/`. Schema migrations generated by `pnpm db:generate`, never hand-authored (per `CLAUDE.md`).
...
- [ ] T006 Run `pnpm db:generate` to produce the migration SQL + snapshot; commit only the generated files (never hand-author, per `CLAUDE.md`)
...
- [ ] T020 [US1] Apply the four filters as additional SQL predicates in `searchPages` at `apps/web/src/server/services/public-content.ts:552-591` — read the parsed frontmatter from the already-loaded `contentSource` in the page iteration (the markdown is in memory at line 578); AND-combine with existing path/status/date predicates; OR-combine multiple values within the same key
...
- [ ] T029 [US2] Apply the four frontmatter filters in `listPagesInternal` at `apps/web/src/server/services/public-content.ts:289-387` — extend the existing SQL condition array (line 311) with new predicates that JOIN or subquery against the parsed frontmatter of each candidate; note that the markdown is currently NOT selected in the batch query (line 343-350), so this requires either (a) reading the markdown per row inside the JS post-filter loop, or (b) adding the markdown to the batch select and filtering in JS (preferred for permission parity — see `public-content.ts:340-350` for the existing batch read pattern)
...
**Goal**: Two new endpoints (`POST /api/v1/search/semantic` and `GET /api/v1/search/semantic/{id}`) backed by the existing `ai_actions` lifecycle, with the `ai.read` scope gating, citation-level results (`chunkId`, `revisionId`, `contentHash`), and the FR-009 permission filter on every result item.
...
- [ ] T036 [US3] Create `apps/web/src/server/services/public-ai.ts` with `submitSemanticSearch(ctx, input)` — wraps the internal `createSemanticSearch` (`apps/web/src/server/services/ai-retrieval.ts:13-32`) but accepts an `api_key` actor; maps the internal `UNAUTHORIZED` to `FORBIDDEN`; returns the `publicSemanticSearchActionSchema` shape
...
- [ ] T058 [US5] Add `batchUpdatePages(ctx, input)` to `apps/web/src/server/services/public-content.ts` — iterate `input.items` (max 50); for each, call the appropriate per-page function from `pages.ts` (`updateProperties` L432-500, `newDraft` L345-430 for frontmatter patches); catch `DomainError` per item and convert to `BatchItemResult`; branch on `input.dryRun` to use a compute-only path that performs all validation (path collision, `STALE_REVISION`, permission) without writing
...
- [ ] T065 Add five new flatteners to `packages/mcp-server/src/shapes.ts:1-241`: `submitSemanticSearchResponse`, `getSemanticSearchResultsResponse`, `getOutboundLinksResponse`, `getNeighborhoodResponse`, `batchUpdatePagesResponse`, `batchSoftDeletePagesResponse` (per `contracts/mcp-tools.md` flattener shapes)
...
- [ ] T081 Run `pnpm --filter @next-wiki/web openapi:generate` one final time; verify `apps/web/public/openapi.json` contains the 7 new entries (semantic POST + GET, pages/{id}/links, graph/neighbors, batch/update, batch/delete) and that no `/v1/*` path uses a `mode=` query parameter (SC-008)
...
- **Foundational (Phase 2)**: Depends on Setup. **BLOCKS** all user stories. T011 depends on T004-T010 (permission wiring). T012-T014 depend on T011 (the `ai.read` scope must exist before the new tests can be written meaningfully). T015-T016 depend on T012 (frontmatter parser is independent of permission scope).
...
- **T011 + T012-T014** can be sequenced (test must run after code) but T011 and T015-T016 can be parallel.
