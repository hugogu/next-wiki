...
Short-lived server-side state for the QR device-code flow. It is deleted after
completion, denial, cancellation, or expiry.

| Field                                        | Type            | Rules                                                                                |
| -------------------------------------------- | --------------- | ------------------------------------------------------------------------------------ |
| `id`                                         | UUID            | Opaque browser-facing registration identifier.                                       |
| `created_by`                                 | UUID FK `users` | Admin who initiated the flow; only that administrator session may poll or cancel it. |
| `domain`                                     | text            | Bounded `feishu` or `lark` account service selection.                                |
| `device_code_encrypted`                      | text            | AES-256-GCM ciphertext only; never returned over HTTP.                               |
| `expires_at`, `last_polled_at`, `created_at` | timestamptz     | Bounded lifecycle and cleanup.                                                       |
