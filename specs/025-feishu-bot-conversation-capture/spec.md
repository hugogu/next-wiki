...
Feishu bot Q&A is brought inside the same conversation pipeline as Wiki AI: a Feishu-bound question creates and continues the same kind of Wiki AI chat session the web chat side pane already uses. Bot Session becomes a thin Feishu-specific wrapper around that session so the existing session-window, rate-limit, and fan-out behavior still applies, but there is no second canonical history record. Captured conversations flow through the existing Raw Conversation capture pipeline and are discoverable through the existing Raw search; admins see one togglable Data Source ("AI Conversations") instead of a Feishu-specific switch, and the existing wiki-AI capture machinery (Conversation Raw page, built-in category, Conversation-specific reader, permission re-checks, audit trail) governs Feishu sessions identically.
...
### User Story 1 - One AI Conversations Data Source Covers Every Channel (Priority: P1)

As an Admin, I want a single Data Source toggle under Content settings to govern whether AI conversations are captured into Raw, regardless of which channel (web Wiki AI, Feishu bot, future bots) the conversation came from, so I can decide retention and discoverability once for the whole product instead of per-channel.

**Why this priority**: Without a unified toggle, admins either over-capture or under-capture channels independently, and the product's "all bots share the same AI core" promise leaks through to retention policy. The toggle is the operator-facing contract for capture.

**Independent Test**: Sign in as Admin, open Content settings → Data Sources, locate AI Conversations, toggle it off. Run a web Wiki AI chat and a Feishu Q&A from two different identities; neither produces a Raw Conversation page. Toggle it on, repeat both channels; both produce Raw Conversation pages under the same Conversation category. Toggle name changes from "Wiki AI Conversations" (or equivalent) to "AI Conversations" and existing default state is preserved across the rename.

**Acceptance Scenarios**:

1. **Given** an Admin opens Content settings, **When** the Data Sources section is displayed, **Then** the section shows a single source labelled `AI Conversations` (not "Wiki AI Conversations" alone and not a separate Feishu-specific row) and the source has an enabled/disabled state.
...
**Independent Test**: As a bound Feishu user, ask a multi-turn question (with pause shorter than the session window). Verify the bot treats the exchange as a single conversation. Then run the same conversation in the Wiki AI chat side pane using the same wiki account, and confirm both surfaces reach the same underlying Wiki AI chat session identifier and the same Raw Conversation page.

**Acceptance Scenarios**:

1. **Given** a bound Feishu user sends the bot a question in a 1:1 chat, **When** the bot answers, **Then** a Wiki AI chat session is created (or resumed, if an in-window prior session exists for the same bound user in the same Feishu chat) and the question, answer, citations, thinking where retained, errors, and timestamps are stored against that session rather than in a Feishu-only history table.
2. **Given** a bound Feishu user @-mentions the bot in a group, **When** the bot answers, **Then** the answering session is the @-mentioner's Wiki AI chat session for that Feishu chat, never a shared group session, and no other bound user in the same chat can read or resume that session through the group.
3. **Given** a Feishu chat has an active session and the bound user sends another message within the configured session window, **When** the bot answers, **Then** the new turn is appended to the same Wiki AI chat session and the conversation history remains coherent.
4. **Given** a Feishu chat session window has elapsed or the bound user has explicitly asked for a new session, **When** the next message arrives, **Then** the bot starts a new Wiki AI chat session for the same bound user, and the new session becomes the canonical record going forward.
5. **Given** the bot defers a long-running answer to an asynchronous AI action, **When** the answer subsequently streams in, **Then** the streaming events are appended to the same Wiki AI chat session in order, and any partial capture status is reconciled only when the session reaches a terminal state.
6. **Given** the Wiki AI chat session reaches a terminal state (completed, failed, cancelled, or expired), **When** the Feishu user receives the final reply, **Then** that terminal state drives the Raw Conversation page's lifecycle status and is visible wherever the captured conversation is shown.

---

### User Story 3 - Bot Session Is a Thin Feishu-Specific Wrapper (Priority: P1)

As an operator maintaining the Feishu integration, I want the Feishu Bot Session to carry only the Feishu-only concerns (chat-window timing, rate limit, chat-id mapping, group fan-out) and delegate the actual conversation record to a Wiki AI chat session, so I don't have to maintain two parallel history stores.
...
**Independent Test**: Start a Feishu Q&A, observe the Bot Session row created for it, and confirm the row only holds Feishu-side state (chat id, binding, session window state, rate-limit counters); the conversation timeline itself lives in a Wiki AI chat session row. Then open the corresponding Raw Conversation page and verify only one history record is referenced.

**Acceptance Scenarios**:

1. **Given** a Feishu Q&A arrives, **When** a Bot Session is created, **Then** it persists only the Feishu-side lifecycle state and a reference to the canonical Wiki AI chat session; the Bot Session MUST NOT store a second timeline of question, answer, citations, or status changes for the conversation.
2. **Given** a Feishu user opens their AI chat history in any channel-aware surface (web chat side pane, AI Chat History list, Search → Raw page), **When** a captured conversation is shown, **Then** every surface resolves to the same Wiki AI chat session identifier and the same Raw Conversation page, never to a separate Bot Session-only row.
3. **Given** the bound user resumes a Feishu conversation via the Feishu channel, **When** prior context is loaded, **Then** that context is read from the Wiki AI chat session, not from any Feishu-side cache.
4. **Given** a Feishu Q&A was answered before AI Conversations capture was enabled, **When** the toggle is later enabled, **Then** that exchange remains only in the Wiki AI chat session record (no retroactive bot-only history appears) and any pre-existing Bot Session rows from before this feature remain as transport wrappers without gaining a parallel history record.
5. **Given** the bound user is unbound or deactivated mid-session, **When** the Bot Session is terminated, **Then** the underlying Wiki AI chat session is also terminated for new turns, the Bot Session retains only a reference to its prior session-id and current state, and no protected content leaks through either surface after termination.
...
1. **Given** AI Conversations is enabled, **When** a Feishu-bound user asks the bot a question, **Then** the audit log records the inbound Feishu attribution, the underlying Wiki AI chat session identifier, the bound wiki user, the action outcome, and a correlation identifier that excludes raw question, answer, and credential text.
...
- A Feishu answer is still streaming when the configured session window expires: the Wiki AI chat session is finalized with whatever state has been preserved, the Bot Session is reset for the next inbound message, and the captured Raw page reflects the preserved state honestly without inventing a final answer that never arrived.
- The bound user unbinds (or is deactivated) while a Feishu conversation is in flight: the active Bot Session and underlying Wiki AI chat session are terminated; the captured Raw page, if already preserved, follows Raw retention policy and is no longer attributed to a now-unbound identity on subsequent access checks.
...
- A group chat contains multiple bound users and one of them @-mentions the bot: only the mentioner's binding drives the Wiki AI chat session; the captured Raw page is permission-scoped to that one bound user and never to the union of the group.
...
- The same captured Feishu conversation is opened from Search, AI Chat History, and a deep link in the Feishu-reply card: every surface resolves to the same Raw Conversation page and the same underlying Wiki AI chat session identifier.
- A Feishu Q&A is started by a bound user but the AI provider is unavailable mid-answer: the Wiki AI chat session records a failure terminal state, the Raw page shows the preserved question and the safe failure message, and search picks it up once indexed.
- A duplicate Feishu event is replayed (anti-replay): the duplicate does not create a second Wiki AI chat session, does not produce a second Raw Conversation page, and does not deliver a second answer.
...
- **FR-001**: The system MUST expose a single Admin-configurable Content Data Source named `AI Conversations` that governs the capture of every AI conversation channel (Wiki AI web chat and Feishu bot) into Raw Conversation pages.
- **FR-002**: The Data Sources section MUST NOT expose a separate per-channel toggle (e.g. a Feishu-specific row) for conversation capture; if additional channels are added later, they MUST be captured under the same `AI Conversations` source.
...
- **FR-005**: While AI Conversations is enabled, every captured Feishu conversation MUST be recorded as exactly one Raw Conversation page assigned to the built-in `Conversation` category.

**Pipeline reuse**

- **FR-006**: The Feishu bot MUST route every bound-user Q&A through the existing Wiki AI chat-session pipeline (004-system-ai-support) so the question, assistant answer, thinking where retained, citations, source references, errors, timestamps, and lifecycle status of a Feishu exchange are stored against a Wiki AI chat session.
- **FR-007**: The Wiki AI chat session created or resumed for a Feishu turn MUST be the canonical conversation record for that exchange; no parallel Feishu-only timeline table or appended history store may be introduced for newly captured conversations.
- **FR-008**: Resuming a Feishu conversation within the configured session window MUST continue the same underlying Wiki AI chat session, and the session-window reset MUST be driven by the Bot Session wrapper while the conversation content remains in the Wiki AI chat session.
- **FR-009**: The bound user who @-mentions the bot in a group chat MUST own the only Wiki AI chat session touched by that turn; other bound users in the same chat MUST NOT widen the scope, read the captured content, or trigger sibling sessions from that one turn.
- **FR-010**: A deferred long-running answer triggered from Feishu MUST stream into and finalize the same Wiki AI chat session that the inbound turn created; intermediate streaming MUST NOT appear as a fresh session.

**Bot Session thin wrapper**

- **FR-011**: The existing Feishu Bot Session entity (019 Key Entities / FR-007 / FR-008) MUST be retained only as a wrapper for Feishu-side concerns: the Feishu chat identifier, the active binding, session-window state, rate-limit counters, group-fan-out state, and the referenced Wiki AI chat session identifier.
- **FR-012**: The Bot Session MUST NOT store a second copy of the conversation timeline (question text, answer text, citations, source identifiers, errors, or status changes) parallel to the Wiki AI chat session.
- **FR-013**: On unbind, admin revocation, or user deactivation, the Bot Session MUST expire immediately, the underlying Wiki AI chat session MUST be terminated for new turns, and any retained Bot Session row MUST contain only the safe transport metadata listed in FR-011.

**Capture, indexing, and search**

- **FR-014**: A captured Feishu conversation MUST be included in Raw keyword search for users permitted to read the corresponding Raw page, with no difference in excerpt length, ranking weight, or result presentation from a web capture.
- **FR-015**: A captured Feishu conversation MUST be included in Raw meaning-based (semantic) search for users permitted to read the corresponding Raw page, under the same capability-id and permission scope as web captures.
- **FR-016**: A search result that points to a captured Feishu conversation MUST identify it as a Conversation result and open the conversation-specific reader on click; it MUST NOT mark the result as a generic raw document or a Feishu-specific custom view.
- **FR-017**: The system MUST enforce Raw read permission on each search access to a captured Feishu conversation, including on resume from Feishu, opening from search, opening from AI Chat History, and opening from a deep link.

**Presentation**

- **FR-018**: A Raw Conversation page produced from a Feishu capture MUST render with the existing conversation-specific reader (questions, answers, thinking where retained, citations, errors, timestamps, status) used for web captures; the channel of origin MUST be carried as page metadata and visible in admin surfaces where applicable but MUST NOT change the reader layout.
- **FR-019**: The product MUST provide localized labels for the Data Sources section (renamed source), audit-channel metadata, and any new channel marker on the Raw Conversation page, consistent with the existing locales for 023 and 019.

**Security and audit**

- **FR-020**: Every Feishu-bot-initiated action that affects a Raw Conversation page MUST be attributed to the bound wiki user and recorded in the audit log with origin `feishu`, alongside the Wiki AI chat session identifier and a correlation identifier that contains no raw question, answer, or credential text.
- **FR-021**: Permission re-checks for reading or resuming a captured Feishu conversation MUST run through the same `can()` chokepoint as every other Raw read, including when the reader is opened from a Feishu-side deep link.
- **FR-022**: A duplicate inbound Feishu event (replay) MUST NOT create a second Wiki AI chat session, a second Raw Conversation page, or a second delivered answer.
- **FR-023**: A Feishu user who lacks read permission for Raw Conversation content MUST NOT discover the existence of a captured Feishu conversation through Search, public navigation, result counts, previews, or direct page open attempts, regardless of their prior participation in the chat.
...
- The capture pipeline writes only to Raw content, which is permission-gated and never part of the publicly readable, ISR-served reader body. The rename of a Data Source label is an Admin UI change and does not affect public documents.
...
- **AI Conversations Data Source**: The renamed, single Content Data Source that governs AI conversation capture across every channel (web Wiki AI and Feishu bot). Holds an enabled/disabled state and an admin-facing label `AI Conversations`. Replaces the prior 023 "Wiki AI Conversations" source conceptually without resetting its stored state.
- **Wiki AI Chat Session**: The existing canonical Wiki AI chat session (from 004-system-ai-support), now also the canonical record for Feishu captures. Holds the conversation timeline, lifecycle status, timestamps, retained thinking, citations, source references, and errors. Channel of origin is recorded as page metadata on the resulting Raw page rather than as a divergent timeline.
- **Feishu Bot Session**: A thin Feishu-specific wrapper around a Wiki AI chat session. Holds the Feishu chat identifier, the active binding, session-window state, rate-limit counters, group-fan-out state, and the referenced Wiki AI chat session identifier. Holds no second copy of the conversation timeline.
...
- **SC-001**: In acceptance testing, 100% of Feishu Q&A exchanges from a bound user, while AI Conversations is enabled, produce exactly one Raw Conversation page assigned to the built-in `Conversation` category and referencing the same underlying Wiki AI chat session identifier as the Q&A itself.
...
- **SC-007**: 100% of data-lineage tests confirm there is exactly one canonical durable conversation record for a captured Feishu Q&A — the Wiki AI chat session — and that no parallel Feishu-only timeline table exists.
- **SC-008**: 100% of audit entries for Feishu-captured conversations record origin `feishu`, the bound wiki user, the Wiki AI chat session identifier, and a correlation identifier free of raw question, answer, or credential text.
- **SC-009**: In configuration tests, an Admin can locate the `AI Conversations` Data Source under Content settings and toggle it on or off in under 30 seconds without editing lower-level system configuration, and the rename from "Wiki AI Conversations" preserves the stored state of existing deployments.
- **SC-010**: In resume tests, a bound Feishu user who sends two turns within the configured session window sees them handled by the same underlying Wiki AI chat session; after the window has elapsed (or after explicit reset), a new turn opens a fresh Wiki AI chat session.
