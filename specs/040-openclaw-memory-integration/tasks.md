...
- [X] T033 [P] [US3] Add recall tests for `own`, `granted`, and `own_and_granted`, post-selection revocation, expired grants, a deleted/unavailable backing page or cited revision, and indistinguishable omissions — covered co-located in `apps/web/src/server/services/agent-memory-management.test.ts` (FR-009 mid-flight revocation via `vi.spyOn` on `listActiveGrantedDestinationIds`, expired-grant, deleted-backing-page cases) and `apps/web/src/server/services/agent-memory.test.ts`, per the repo's co-located-test convention rather than a separate `__tests__/recall-grants.test.ts`
- [X] T034 [P] [US3] Add management tests proving bearer keys cannot create grants/promotions and direct shared writes fail — covered in `apps/web/src/server/services/agent-memory-management.test.ts` ("rejects grant and promotion attempts from a non-owner (agent) actor", asserting `createSharedDestination`/`createGrant`/`promote` all reject `FORBIDDEN` for an api-key actor), not a separate `__tests__/grants-and-promotions.test.ts`
- [X] T035 [P] [US3] Add bridge tool/prompt tests for authority checks, bounded escaped citations, and denied save/forget approvals in `packages/openclaw-memory-bridge/tests/tools-and-prompt.test.ts` (9 tests: tool registration gating, citation escaping, bounded enrichment record count, safe-skip on missing session/client errors)

### Implementation for User Story 3

- [X] T036 [US3] Extend `POST /api/v1/memory/recall` to derive sources from private destination plus active grants and recheck each output in `apps/web/app/api/v1/memory/recall/route.ts` and `apps/web/src/server/services/agent-memory.ts`
- [X] T037 [US3] Implement owner-only shared namespace/read-grant and curated-promotion routes: `apps/web/app/api/api-keys/agent-memory/shared-destinations/route.ts` (list/create shared destinations), `apps/web/app/api/api-keys/agent-memory/connections/[connectionId]/read-grants/route.ts` (create grant), `.../read-grants/[grantId]/route.ts` (revoke grant), `apps/web/app/api/api-keys/agent-memory/read-grants/route.ts` (list all grants), and `apps/web/app/api/api-keys/agent-memory/promotions/route.ts` (promote)
- [X] T038 [US3] Add sharing/promotion controls to the User Center — implemented as a new standalone `apps/web/src/components/user-center/AgentMemorySharing.tsx` (shared destinations, grants, promotion form) alongside a new `AgentMemoryConnections.tsx` (connection CRUD), both wired into `apps/web/app/(user)/user-center/api-keys/page.tsx`, rather than extending the legacy `ApiKeyList.tsx`/`ApiKeyCreateDialog.tsx` — keeps the connection-scoped credential model (P11: single feature entry point) separate from generic API keys; `apps/web/src/server/services/api-keys.ts`'s `list()` was fixed to exclude connection-backed keys from the legacy list to avoid duplicate entries (found via live browser verification)
- [X] T039 [US3] Implement optional static OpenClaw tools, mutation approvals, and authorized second-phase prompt enrichment in `packages/openclaw-memory-bridge/src/tools.ts` and `packages/openclaw-memory-bridge/src/prompt-enrichment.ts`; manifest (`openclaw.plugin.json`) updated with the 4 tool contracts/metadata; `config.ts` gained `tools.enabled`/`sharedRecall.enabled`
...
- [X] T047 Create/rename the managed generic Agent Memory guide and add the managed OpenClaw bridge guide without live configuration in `apps/web/src/server/services/setup-sample-pages.ts` / `setup-sample-page-definitions.ts`. The former single Hermes-only guide's constants were split three ways: `HERMES_PAGE_TITLE/SOURCE` (renamed, unchanged content, still at `integrations/hermes`), a new generic `AGENT_MEMORY_PAGE_TITLE/SOURCE` (adapter-agnostic: connections, credentials, recall, sharing), and a new `OPENCLAW_PAGE_TITLE/SOURCE`. **Deviation from the obvious design**: the generic guide could *not* reuse the historic `LEGACY_AGENT_MEMORY_PAGE_PATH` (`help/agent-memory`) as its canonical path — feature 035 permanently retains a moved page's old slug as a redirect alias, so once the one-time Hermes migration moves content away from that path, the address can never be reused for a different page (confirmed by a real `ADDRESS_UNAVAILABLE`-style failure in a test before the fix). The generic guide instead lives at `help/agent-memory-guide`. Also fixed a related pre-existing latent bug the new page count exposed: the legacy-migration guard only checked for the generic `SAMPLE_PAGE_MARKER`, which would have made a reinit mistake the already-migrated generic guide (which also carries that marker) for stale Hermes content; now requires the found page's title to match the exact definition being processed.
- [X] T048 Add page-specific/help-navigation public cache tags and targeted invalidation for changed managed guide representations. Touched files differ from the task's guess: the actual create/update invalidation call setup-sample-pages relies on lives in `revisions.ts`'s `publish()` (not `pages.ts`, which only invalidates on restore/remove/setVisibility/updateProperties/moveToSpace — none of which fire on the common create/update path), so `publish()` gained an opt-in `skipPublicCacheInvalidation` flag (default off; every other caller unaffected), and `apps/web/src/server/services/public-content.ts` (not `pages.ts`) is where the actual page-tree/navigation cache lives, so that's where `HELP_NAVIGATION_CACHE_TAG` was added. `public-cache.ts` gained `HELP_NAVIGATION_CACHE_TAG` and `invalidateManagedGuideCache()` (tag-only revalidation, deliberately omitting `invalidatePublicContentCache`'s `revalidatePath('/', 'layout')` full site-wide rebuild, since a setup/reinit guide write is infrequent and non-interactive). **Scope note**: this is tag-scoped (all guides share `PUBLIC_CONTENT_CACHE_TAG`), not per-page-ID granular — true per-page tagging would require restructuring the shared, hot-path `unstable_cache` wrappers in `pages.ts` used by every public page in the wiki, which was judged disproportionate risk/complexity for a documentation-guide feature.
- [X] T049 [P] Add collision/rerun/disabled-update and targeted-cache-invalidation tests in `apps/web/src/server/services/setup-sample-pages.test.ts` (29 tests, incl. a new test proving the legacy migration retires its old path rather than letting the generic guide reuse it) and `apps/web/src/server/cache/public-cache.test.ts` (4 tests: `invalidateManagedGuideCache` never calls `revalidatePath`, `invalidatePublicContentCache` still does). Also added one targeted test in `apps/web/src/server/services/revisions.test.ts` for the new `skipPublicCacheInvalidation` flag itself.
- [X] T050 Update managed help pages (done in T047) and adapter documentation: `packages/hermes-memory-provider/README.md` (points to the new Agent memory connections UI instead of the legacy "Memory provider" preset, which still works but is no longer the recommended path) and `packages/openclaw-memory-bridge/README.md` (documents the `tools`/`sharedRecall` config keys added in Phase 5, plus connection setup). `specs/040-openclaw-memory-integration/quickstart.md` gained dated "Result" sections recording what's actually been verified for Phases 1-5 and this phase, and an explicit note that the import/migration (Phase 6) verification is still pending.
- [X] T051 Ran `pnpm --filter @next-wiki/web openapi:generate` — no diff (Phase 5/7 work in this pass didn't change any route contract) — and confirmed the `openapi-schemas.test.ts` sync suite (2125 tests) still passes.
- [X] T052 [P] Added `.github/workflows/publish-openclaw-memory-bridge.yml` (build, typecheck, test, manifest validation via a new `packages/openclaw-memory-bridge/scripts/validate-manifest.mjs`, `npm pack`, clean-install into a scratch project with the real `openclaw` peer dependency, and runtime inspection via `scripts/inspect-runtime.mjs` before publish). Deferred `publish-openclaw-memory-migrate.yml` to Phase 6, since that package doesn't exist yet. **The clean-install/runtime-inspection gate immediately caught a real, previously-shipped packaging bug**: `tsconfig.base.json` uses `moduleResolution: "Bundler"`, which lets `tsc` silently emit relative imports/re-exports without a `.js` extension; that output is invalid for Node's native ESM resolver, so every real (non-monorepo, non-bundled) install of this plugin would have failed at load time with `ERR_MODULE_NOT_FOUND`. Fixed by adding explicit `.js` extensions to all nine relative import/export specifiers across `packages/openclaw-memory-bridge/src/*.ts` (a no-op for typecheck/bundled consumers, required for the published dist output) — verified by a real `npm pack` + clean install + dynamic `import()` of the packed tarball, both before (reproduced the failure) and after (passes) the fix.
- [X] T053 Add a browser/API end-to-end scenario for owner provisioning, three independent adapter credentials, private isolation, shared-promotion grant/revocation, and OpenAPI visibility in `apps/web/e2e/agent-memory-integrations.spec.ts`. Runs a real `playwright test` against a dedicated e2e Postgres + built server (no mocks): creates 3 connections via the session-authenticated management routes, confirms `GET /api/v1/memory/connection` never echoes a secret, saves a private record through each and confirms cross-connection recall returns empty, promotes one record into a new shared destination, grants a second connection read access and confirms `scope: "granted"` recall returns the promoted record, revokes the grant and confirms recall returns empty again, confirms an agent (Bearer) credential gets 403 attempting to create its own grant, and confirms both `/api/openapi.json` and the agent-facing `/api/public-openapi.json` expose the `/v1/memory/*` contract while owner-management routes stay session-only/unpublished. Passed on first real run except one wrong expected status code (403, not 401, for the non-owner rejection), fixed and reverified green.
- [X] T054 Ran the focused web suite (agent-memory + api-keys + setup-sample-pages + public-cache + revisions + openapi-schemas: 12 files, 2223 tests, all green), the bridge suite (6 files, 47 tests, all green), full-workspace `pnpm typecheck` (5/5 tasks) and `pnpm --filter @next-wiki/web lint` — all clean. Confirmed FR-011/FR-012 by inspecting `apps/web/package.json`'s merged `dependencies`/`devDependencies`: zero entries matching `hermes` or `openclaw`. **Skipped two sub-items deliberately, not silently**: (1) the Hermes Python suite — untouched this pass (only its README changed, no source), and (2) `docker compose up -d --build` — this worktree's own compose project isn't running (`docker compose ps` here is empty), but the *main* checkout's `next-wiki-web`/`next-wiki-db` containers are already up and bound to the default ports this worktree's compose file would also claim; starting a second stack from here risked a port conflict with another active session for a check already covered by the real-Postgres-backed vitest and Playwright runs above, so it was not run. Recorded two-connection, capture-recovery, and grant-revocation outcomes in `quickstart.md` per-section as each was verified (see T033-T039); recorded the guide-cache outcome under T047/T048/T049 above and in `quickstart.md`. Import-preview remains genuinely pending Phase 6 (not yet implemented) — `quickstart.md`'s final section says so explicitly rather than fabricating a result.
