...
describe('registerAgentMemoryTools', () => {
  ...
  it('search tool forces own scope when sharedRecall is disabled, regardless of the requested scope', async () => {
    ...
    const result = await search.execute('call-1', { query: 'decision', scope: 'granted' });
