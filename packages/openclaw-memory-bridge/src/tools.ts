...
export function registerAgentMemoryTools(api: OpenClawPluginApi, config: BridgeConfig, client: WikiApiClient): void {
  ...
  const allowedScopes: RecallScope[] = config.sharedRecall.enabled ? ['own', 'granted', 'own_and_granted'] : ['own'];

  const searchTool: AnyAgentTool = {
    name: 'agent_memory_search',
    label: 'Search Agent Memory',
    description: config.sharedRecall.enabled
      ? 'Search this connection\'s private Agent memory destination and any owner-granted shared destinations.'
      : 'Search this connection\'s private Agent memory destination.',
    parameters: Type.Object({
      query: Type.String({ maxLength: MAX_QUERY_CHARS }),
      limit: Type.Optional(Type.Integer({ minimum: 1, maximum: 10 })),
      ...(config.sharedRecall.enabled
        ? { scope: Type.Optional(Type.Union(allowedScopes.map((scope) => Type.Literal(scope)))) }
        : {}),
    }),
    async execute(_id, params) {
      return handleClientError(async () => {
        const { query, limit, scope } = params as { query: string; limit?: number; scope?: RecallScope };
        const effectiveScope = config.sharedRecall.enabled && scope ? scope : 'own';
        const response = await client.recall(query, limit ?? 5, effectiveScope);
  ...
  const saveTool: AnyAgentTool = {
    ...
    description: 'Explicitly save approved content to this connection\'s private Agent memory destination.',
    parameters: Type.Object({
  ...
  const forgetTool: AnyAgentTool = {
    ...
    description: 'Reversibly forget one record from this connection\'s private Agent memory destination.',
    parameters: Type.Object({
  ...
  const statusTool: AnyAgentTool = {
    ...
    description: 'Report this connection\'s Agent memory diagnostics (no credentials or content).',
    parameters: Type.Object({}),
    async execute() {
      return handleClientError(async () => {
