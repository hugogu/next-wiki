...
const ERROR_ACTIONS: Record<number, { code: string; message: string }> = {
  401: { code: 'unauthorized', message: 'The credential is invalid or revoked. Rotate or reconfigure the memory-provider API key.' },
  403: { code: 'forbidden', message: 'The credential lacks a required memory scope.' },
...
export class WikiApiClient {
  ...
  recall(query: string, limit: number): Promise<Record<string, unknown>> {
    return this.request('POST', '/memory/recall', { query, limit });
  }
