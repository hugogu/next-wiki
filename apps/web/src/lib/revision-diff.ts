...
export type DiffRow =
  | { left?: SourceLine; right?: SourceLine; kind: DiffKind }
  | { kind: 'collapsed'; leftRange: string; rightRange: string };
...
export function tokenizeSource(source: string, ignoreWhitespace = false): SourceLine[] {
  ...
  const tokens = lines.map((text, index) => ({
    number: index + 1,
    text,
    compareKey: ignoreWhitespace ? text.replace(/\s/g, '') : text,
  }));
  ...
  for (let index = 0; index < changes.length; index += 1) {
    ...
    if (!change.added && !change.removed) {
      values.forEach((_, offset) =>
        rows.push({
          left: left[leftIndex + offset],
          right: right[rightIndex + offset],
          kind: 'unchanged',
        }),
      );
    ...
    if (change.removed && next?.added) {
      ...
      for (let i = 0; i < shared; i += 1)
        rows.push({ left: values[i], right: added[i], kind: 'changed' });
      ...
      for (let i = 0; i < shared; i += 1)
        rows.push({ left: removed[i], right: values[i], kind: 'changed' });
      ...
      leftIndex += removed.length;
      rightIndex += values.length;
      index += 1;
    } else if (change.removed) {
      values.forEach((line) => rows.push({ left: line, kind: 'removed' }));
      leftIndex += values.length;
    } else {
      values.forEach((line) => rows.push({ right: line, kind: 'added' }));
      rightIndex += values.length;
    }
  }
  return rows;
...
function range(rows: Array<Extract<DiffRow, { kind: Exclude<DiffKind, 'collapsed'> }>>): {
  leftRange: string;
  rightRange: string;
} {
...
export function withContext(rows: DiffRow[], context: number | 'full'): DiffRow[] {
  ...
  rows.forEach((row, index) => {
    if (visible.has(index)) {
      if (hidden.length) {
        output.push({ kind: 'collapsed', ...range(hidden) });
        hidden = [];
      }
      output.push(row);
...
export function changedLineNumbers(rows: DiffRow[]): { left: Set<number>; right: Set<number> } {
  ...
  rows.forEach((row) => {
    if (row.kind !== 'unchanged' && row.kind !== 'collapsed') {
