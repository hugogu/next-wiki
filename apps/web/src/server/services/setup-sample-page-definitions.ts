...
\`baseUrl\` is the next-wiki origin only — never append \`/api/v1\`; a copied API base is auto-normalized. \`apiKeyRef\` accepts a plain string or an \`env\`/\`file\`/\`exec\` SecretRef; keep the key out of ordinary config files. The key needs the Agent Memory \`memory.write\` scope for synchronization and \`memory.read\` plus \`view\` for the \`next_wiki_search\`/\`next_wiki_get\` tools. Restart the gateway after changing config, then verify with \`next_wiki_status\`. If \`memoryPath\` is omitted, it follows the active OpenClaw workspace (including profile and workspace overrides) and appends \`memory/\`; the workspace-root \`MEMORY.md\` is mirrored too. The resolved workspace must exist; an accidental profile/workspace mismatch is reported as degraded status instead of silently syncing nothing. Missing memory paths are treated as empty until memory-core creates them. Files larger than 512 KB are skipped with a warning instead of stalling the whole run. The plugin scans root Markdown files and the standard \`entities/\`, \`concepts/\`, \`syntheses/\`, \`sources/\`, \`reports/\`, and \`_views/\` trees, plus memory-core Markdown; transient \`memory/working/\` traces and duplicate bridge content are skipped. Attachments and \`.openclaw-wiki\` state are excluded.
