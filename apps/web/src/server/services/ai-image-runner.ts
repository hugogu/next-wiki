...
async function summarizeForIllustration(actionId: string, title: string, source: string): Promise<string | null> {
  ...
  for await (const event of createAiProviderAdapter(await providerRuntime(provider.id)).streamText({
    ...
    system: 'You write a concise visual scene description for a single thematic illustration. Output only the description, under 1000 characters, no Markdown, no preamble.',
    messages: [{ role: 'user', content: `Summarize the following wiki page titled "${title}" into a vivid visual scene for a header illustration. Focus on concrete imagery; avoid text, UI and logos.\n\n${source.slice(0, 8_000)}` }],
    maxOutputTokens: 500,
    temperature: 0.4,
...
/** Shared provider runner for the pg-boss image job and the pg-boss Wiki AI tool loop. */
export async function executeImageGenerationAction(actionId: string): Promise<void> {
  const input = await readActionInput<ImageGenerationInput>(actionId);
  ...
  if (await isCancellationRequested(actionId)) throw new DomainError('CANCELLED', 'Image generation was cancelled');
  const prompt = (summary ? `${baseInstruction}Topic: ${summary}` : `${baseInstruction}Topic context:\n\n${source}`).slice(0, IMAGE_PROMPT_MAX_CHARS);
...
/**
 * Execute a child image action from the already-backgrounded Wiki AI tool
 * workflow. This deliberately does not enqueue a second pg-boss job: the
 * caller is itself the `wiki_question` pg-boss worker. It still gives the
 * child action the same started/terminal lifecycle as the regular queue path.
 */
export async function runInlineImageGenerationAction(actionId: string): Promise<void> {
  await startAction(actionId);
  try {
    await executeImageGenerationAction(actionId);
  } catch (error) {
    const details = error instanceof DomainError
      ? { code: error.code, message: error.message, status: error.code === 'CANCELLED' ? 'cancelled' as const : 'failed' as const }
      : (() => {
        const normalized = normalizeProviderError(error);
        return {
          code: normalized.code,
          message: normalized.message,
          status: normalized.code === 'CANCELLED' ? 'cancelled' as const : 'failed' as const,
        };
      })();
    await finishAction(actionId, details.status, {
      errorCode: details.code,
      errorMessage: details.message,
      errorDetail: error instanceof Error ? error.stack ?? null : String(error),
    });
    throw error;
  }
}
