...
const rawEntries = vi.hoisted(() => ({ createEntry: vi.fn(), replaceEntry: vi.fn() }));
...
vi.mock('@/server/services/spaces', () => ({ resolveSpace: vi.fn(async () => ({ id: 'raw-space', kind: 'raw', slug: 'raw' })) }));
...
describe('agent memory source documents', () => {
  ...
  it('creates a source document on the shared Raw/Page/Revision pipeline and retains exact Markdown', async () => {
    ...
    expect(rawEntries.createEntry).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
      path: expect.stringContaining('agent-memory/namespace-id/memory-wiki/entities/alex-'),
      content,
  ...
  it('returns unchanged for the current digest and writes a full replacement for changed content', async () => {
    ...
    expect(rawEntries.replaceEntry).toHaveBeenCalledWith(expect.anything(), 'page-id', expect.objectContaining({ content: second }));
  });

  it('searches only visible current revisions and reports incomplete space coverage', async () => {
    publicContent.searchPages.mockResolvedValue({ items: [{ page: { id: 'page-id', latestRevision: { id: 'revision-id', contentHash: 'a'.repeat(64) }, spaceSlug: 'wiki', title: 'Profile', path: 'profile', canonicalUrl: 'https://wiki.example/profile' }, excerpt: 'Alex', score: 1 }] });
    ...
    expect(result).toMatchObject({ coverage: { wiki: true, raw: false, generated: false, complete: false }, results: [{ revisionId: 'revision-id' }] });
  });

  it('bounds a selected page read and keeps the current revision citation', async () => {
    publicContent.getPageById.mockResolvedValue({ id: 'page-id', spaceSlug: 'wiki', path: 'profile', title: 'Profile', contentSource: '# Profile\nLong body', canonicalUrl: 'https://wiki.example/profile', latestRevision: { id: 'revision-id', contentHash: 'a'.repeat(64) } });
