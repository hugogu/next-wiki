...
type DefinitionFields = Pick<
  typeof schema.scheduledAiJobs.$inferSelect,
  | 'id'
  | 'name'
  | 'taskDescription'
  | 'scheduleCron'
...
function executionOwnerId(ctx: PermCtx): string {
  ...
  if (!userId)
    throw new DomainError('FORBIDDEN', 'A signed-in user is required to configure a Job');
  return userId;
}
...
export function validateScheduledAiJobSchedule(scheduleCron: string, timeZone: string): void {
  if (scheduleCron.trim().split(/\s+/).length !== 5) {
    throw new DomainError(
      'BAD_REQUEST',
      'SCHEDULE_INVALID: schedules require exactly five cron fields',
    );
  }
  try {
...
export function nextScheduledAiJobOccurrence(
  scheduleCron: string,
  timeZone: string,
  after = new Date(),
): Date {
  ...
  try {
    return CronExpressionParser.parse(scheduleCron, { tz: timeZone, currentDate: after })
      .next()
      .toDate();
  } catch {
...
export function buildScheduledAiJobSnapshot(
  definition: DefinitionFields,
): ScheduledAiJobDefinitionSnapshot {
  return Object.freeze(
    scheduledAiJobDefinitionSnapshotSchema.parse({
...
async function validateOwnerAndScope(runAsUserId: string, targetScope: unknown) {
  ...
  if (!owner || owner.status !== 'active' || owner.deletedAt) {
    throw new DomainError(
      'BAD_REQUEST',
      'SCHEDULED_JOB_OWNER_INELIGIBLE: execution owner must be active',
    );
  }
  ...
  if (spaces.length !== scope.spaceIds.length)
    throw new DomainError('BAD_REQUEST', 'SCHEDULED_JOB_SCOPE_INVALID: space does not exist');
  if (skills.some((skill) => !skill || !skill.enabled)) {
    throw new DomainError(
      'BAD_REQUEST',
      'SCHEDULED_JOB_SKILL_INVALID: selected skill is unavailable',
    );
  }
  return scope;
}
...
function definitionView(row: typeof schema.scheduledAiJobs.$inferSelect) {
  return {
    ...
    targetScope: scheduledAiJobScopeSchema.parse(row.targetScope),
      ...
      .values({
        ...
        nextRunAt:
          parsed.status === 'enabled'
            ? nextScheduledAiJobOccurrence(parsed.scheduleCron, parsed.timeZone, now)
            : null,
        createdByUserId: getActorUserId(ctx),
        updatedByUserId: getActorUserId(ctx),
      })
      .returning();
...
export async function listScheduledAiJobs(
  ctx: PermCtx,
  ...
  const rows = await db
    ...
    .where(
      and(
        input.status ? eq(schema.scheduledAiJobs.status, input.status) : undefined,
        input.q ? ilike(schema.scheduledAiJobs.name, `%${input.q.trim()}%`) : undefined,
      ),
    )
    .orderBy(desc(schema.scheduledAiJobs.updatedAt))
...
export async function getScheduledAiJob(ctx: PermCtx, id: string) {
  assertManager(ctx);
  const row = await db.query.scheduledAiJobs.findFirst({
    where: eq(schema.scheduledAiJobs.id, id),
  });
...
export async function updateScheduledAiJob(ctx: PermCtx, id: string, input: unknown) {
  assertManager(ctx);
  const current = await db.query.scheduledAiJobs.findFirst({
    where: eq(schema.scheduledAiJobs.id, id),
  });
  ...
  const candidate = scheduledAiJobCreateSchema.parse({
    ...current,
    ...update,
    targetScope: update.targetScope ?? current.targetScope,
  });
  ...
  const [row] = await db
    ...
    .set({
      ...candidate,
      targetScope: scope,
      runAsUserId: current.runAsUserId,
      normalizedName: normalizeName(candidate.name),
      nextRunAt,
      definitionVersion: executionChanged
...
export async function retireScheduledAiJob(ctx: PermCtx, id: string) {
  ...
  await db
    ...
    .where(
      and(
        eq(schema.scheduledAiJobRuns.jobId, id),
        inArray(schema.scheduledAiJobRuns.status, ['queued', 'running']),
      ),
    );
  return definitionView(row);
}
...
async function createRun(
  job: typeof schema.scheduledAiJobs.$inferSelect,
  trigger: 'schedule' | 'manual' | 'recovery',
  scheduledFor: Date | null,
) {
  const [run] = await db
    .insert(schema.scheduledAiJobRuns)
    .values({
      jobId: job.id,
      trigger,
      scheduledFor,
  ...
  await enqueue(
    QUEUES.scheduledAiRun,
    { runId: run.id },
    { singletonKey: run.id, singletonSeconds: 60 },
  );
  return runView(run);
}
...
export async function runScheduledAiJobNow(ctx: PermCtx, id: string) {
  assertManager(ctx);
  const job = await db.query.scheduledAiJobs.findFirst({
    where: eq(schema.scheduledAiJobs.id, id),
  });
  if (!job || job.status === 'retired')
    throw new DomainError('NOT_FOUND', 'Scheduled AI job not found');
  try {
...
export async function listScheduledAiJobRuns(
  ctx: PermCtx,
  jobId: string,
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(schema.scheduledAiJobRuns.jobId, jobId),
    ...
    .orderBy(desc(schema.scheduledAiJobRuns.queuedAt))
    .limit(input.limit ?? 25)
    .offset(input.offset ?? 0);
...
export async function getScheduledAiJobRun(ctx: PermCtx, jobId: string, runId: string) {
  assertManager(ctx);
  const run = await db.query.scheduledAiJobRuns.findFirst({
    where: and(eq(schema.scheduledAiJobRuns.id, runId), eq(schema.scheduledAiJobRuns.jobId, jobId)),
  });
  if (!run) throw new DomainError('NOT_FOUND', 'Scheduled AI job run not found');
  const proposals = await db
    .select({
      id: schema.aiToolChangeProposals.id,
      title: schema.aiToolChangeProposals.title,
      status: schema.aiToolChangeProposals.status,
    })
    .from(schema.aiToolChangeProposals)
    .where(eq(schema.aiToolChangeProposals.scheduledAiJobRunId, run.id));
  return {
    ...runView(run),
    actionId: run.aiActionId,
    proposalLinks: proposals.map((proposal) => ({
      ...proposal,
      href: `/admin/ai/tools/proposals/${proposal.id}`,
    })),
  };
}

export async function listAllScheduledAiJobRuns(
  ctx: PermCtx,
...
export async function duplicateScheduledAiJob(ctx: PermCtx, id: string) {
  assertManager(ctx);
  const source = await db.query.scheduledAiJobs.findFirst({
    where: eq(schema.scheduledAiJobs.id, id),
  });
  ...
  while (
    await db.query.scheduledAiJobs.findFirst({
      where: eq(schema.scheduledAiJobs.normalizedName, normalizeName(name)),
    })
  ) {
    suffix += 1;
  ...
  const [copy] = await db
    ...
    .values({
      ...
      runAsUserId: executionOwnerId(ctx),
      status: 'paused',
      definitionVersion: 1,
      createdByUserId: getActorUserId(ctx),
      updatedByUserId: getActorUserId(ctx),
...
export async function tickScheduledAiJobs(now = new Date()) {
  const due = await db
    ...
    .from(schema.scheduledAiJobs)
    .where(
      and(eq(schema.scheduledAiJobs.status, 'enabled'), lte(schema.scheduledAiJobs.nextRunAt, now)),
    )
    .limit(100);
  for (const job of due) {
    ...
    const [claimed] = await db
      ...
      .where(
        and(
          eq(schema.scheduledAiJobs.id, job.id),
          eq(schema.scheduledAiJobs.nextRunAt, occurrence),
        ),
      )
      .returning();
    if (!claimed) continue;
