...
import { runHermesMemoryCapture } from './hermes-memory-capture';
...
export async function registerJobs(boss: PgBoss): Promise<void> {
  ...
  await boss.work(QUEUES.hermesMemoryCapture, { batchSize: 10 }, async (jobs: JobBatch) => {
    for (const job of jobs) await runHermesMemoryCapture(job.data);
  });
