...
export function ScheduledAiJobList({ jobs, runs }: Props) {
  ...
  </SettingsTabs>{creating && <ModalDialog title="Create scheduled AI job" description="AI changes always require review before becoming durable." onClose={() => setCreating(false)}><ScheduledAiJobForm onCancel={() => setCreating(false)} onSaved={(job) => { setCreating(false); router.push(`/admin/ai/jobs/${job.id}`); router.refresh(); }} /></ModalDialog>}</>;
}
