    ...
    <div className="space-y-md">
      ...
      <div className="space-y-sm">
        {results.slice((page - 1) * pageSize, page * pageSize).map((result) => (
          <a key={result.pageId} href={result.canonicalUrl ?? getSpaceHref(readerSpaceFromSlug(result.spaceSlug), result.slug || result.path)} className="block rounded-lg border border-border bg-surface p-md hover:bg-surface-elevated">
