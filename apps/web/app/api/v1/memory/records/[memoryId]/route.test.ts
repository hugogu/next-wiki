...
vi.mock('@/server/services/agent-memory', () => ({ forget }));

import * as route from './route';

describe('DELETE /api/v1/memory/records/:memoryId', () => {
  it('rejects a non-UUID memory id before reaching the service', async () => {
    const response = await route.DELETE(
      new NextRequest('http://localhost/api/v1/memory/records/not-a-uuid', {
        method: 'DELETE',
  ...
  it('accepts an empty body when the optional reason is omitted', async () => {
    ...
    const response = await route.DELETE(
      new NextRequest(`http://localhost/api/v1/memory/records/${memoryId}`, {
        method: 'DELETE',
        headers: { 'x-next-wiki-memory-provider-version': '0.1.0' },
      }),
