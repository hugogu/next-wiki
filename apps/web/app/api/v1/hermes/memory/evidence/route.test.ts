...
vi.mock('@/server/services/hermes-memory', () => ({ submitEvidenceCapture }));

import * as route from './route';

describe('POST /api/v1/hermes/memory/evidence', () => {
  it('returns an origin-relative poll URL for the versioned API', async () => {
    ...
    const response = await route.POST(
      new NextRequest('http://localhost/api/v1/hermes/memory/evidence', {
        method: 'POST',
    ...
    await expect(response.json()).resolves.toMatchObject({
      captureId,
      pollUrl: `/api/v1/hermes/memory/evidence/${captureId}`,
    });
  });
});
