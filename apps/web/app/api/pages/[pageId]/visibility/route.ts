...
const bodySchema = z.object({ visibility: z.enum(['public', 'restricted']) });
...
export async function PUT(request: Request, { params }: { params: Promise<{ pageId: string }> }) {
  ...
  if (!parsedBody.success) return apiError('BAD_REQUEST', 'Visibility must be public or restricted', 400);
  try {
