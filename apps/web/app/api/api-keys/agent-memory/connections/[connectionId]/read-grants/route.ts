...
async function handleGET(_request: NextRequest, { params }: Context) {
  ...
  try {
    return NextResponse.json(await grants.listGrants(await createApiContext(), connectionId, 'read'), { headers: { 'Cache-Control': 'private, no-store' } });
  } catch (error) {
...
async function handlePOST(request: NextRequest, { params }: Context) {
  const { connectionId } = await params;
  if (!uuidSchema.safeParse(connectionId).success) return NextResponse.json({ code: 'NOT_FOUND', message: 'Not found' }, { status: 404 });
  const parsed = parseJson(agentMemoryGrantCreateSchema.extend({ capability: agentMemoryGrantCreateSchema.shape.capability.default('read') }), await request.json().catch(() => ({})));
  if (!parsed.ok) return NextResponse.json({ code: 'BAD_REQUEST', message: formatZodError(parsed.error) }, { status: 400 });
  try {
    return NextResponse.json(await grants.createGrant(await createApiContext(), { connectionId, destinationId: parsed.data.destinationId, capability: 'read', expiresAt: parsed.data.expiresAt }), { status: 201 });
  } catch (error) {
    return handleApiError(error);
  }
}
