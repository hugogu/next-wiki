...
import { getScheduledAiJob, retireScheduledAiJob, updateScheduledAiJob } from '@/server/services/scheduled-ai-jobs';
...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  try { return NextResponse.json(await getScheduledAiJob(await createApiContext(), id), { headers: noStore }); }
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try { return NextResponse.json(await updateScheduledAiJob(await createApiContext(), id, parsed.data), { headers: noStore }); }
  catch (error) { return error instanceof DomainError ? mapDomainError(error) : internalError(); }
}
...
export async function DELETE(_request: NextRequest, { params }: Params) {
  const { id } = await params;
  if (!idSchema.safeParse(id).success) return apiError('NOT_FOUND', 'Not found', 404);
  try { return NextResponse.json(await retireScheduledAiJob(await createApiContext(), id), { headers: noStore }); }
  catch (error) { return error instanceof DomainError ? mapDomainError(error) : internalError(); }
}
