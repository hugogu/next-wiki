...
type Params = { params: Promise<{ id: string }> }; const idSchema = z.string().uuid(); const noStore = { 'cache-control': 'no-store' };
...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  const parsed = scheduledAiJobRunListFilterSchema.safeParse(Object.fromEntries(request.nextUrl.searchParams.entries()));
  if (!parsed.success) return apiError('BAD_REQUEST', 'Invalid run filters', 400);
  try { return NextResponse.json(await listScheduledAiJobRuns(await createApiContext(), id, parsed.data), { headers: noStore }); }
...
export async function POST(_request: NextRequest, { params }: Params) {
  const { id } = await params; if (!idSchema.safeParse(id).success) return apiError('NOT_FOUND', 'Not found', 404);
  try { return NextResponse.json(await runScheduledAiJobNow(await createApiContext(), id), { status: 202, headers: noStore }); }
  catch (error) { return error instanceof DomainError ? mapDomainError(error) : internalError(); }
}
